...
> **Performance Note:** This class uses `__slots__` for memory optimization with fast access to `id` and `g` attributes while storing most data in a dictionary for flexibility.
